...
import net.minesky.data.MemberData;
...
public class GuildHandler {
    ...
    public static void saveGuildas() {
        guildas.values().forEach(g -> {
                    coll.replaceOne(new Document("_id", g.getId()),
                            guildToDoc(g),
    ...
    public static void createGuilda(String id, String name, String tag, Player lider) {
        Guilds guilda = new Guilds(id, name, tag, lider.getUniqueId(), 0, 0);
    ...
    public void setBanner(ItemStack item, Guilds guild) {
